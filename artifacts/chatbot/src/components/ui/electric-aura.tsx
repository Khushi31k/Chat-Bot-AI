// ShaderBackground — WebGL orb shader (purple palette for ELLA)
...
// Purple palette — deep black → ELLA indigo → electric violet → soft lavender
const UNIFORMS = {
  colors: [
    [0, 0, 0],                           // black core
    ...
    [0.937, 0.922, 1.000],               // soft lavender white
    [0.937, 0.922, 1.000],
    [0.937, 0.922, 1.000],
    [0.937, 0.922, 1.000],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(([e]) => { inView = e?.isIntersecting ?? true; if (inView) requestRender(); else { cancelAnimationFrame(raf); raf = 0; lastNow = null } }); io.observe(canvas)
    const onVis = () => { visible = document.visibilityState === "visible"; if (visible) requestRender(); else { cancelAnimationFrame(raf); raf = 0; lastNow = null } }
